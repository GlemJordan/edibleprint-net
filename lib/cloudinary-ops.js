...
// is a real order record — searchOrders()'s expression below is a coarse
// pre-filter that also matches unrelated resources (customer photo uploads,
// each order's own production-slip.pdf/notes/print-design*.pdf siblings),
// so every caller of searchOrders() must still regex-validate each
// public_id against this before treating it as an order. One constant,
// shared by every caller (the admin order list, the backup, the manual-
// order duplicate check), instead of three copies that could drift.
export const ORDER_PUBLIC_ID_RE = /^edibleprint\/orders\/([^/]+)\/order$/;

...
//
// ON BY DEFAULT in the editor (app/page.js) — a design that doesn't fill its
// shape (e.g. an illustration on a white background) gives the customer no
// reference for where to cut without this line, so a clean sheet is the
// worse default; customers who don't want it switch it off there. This file
// doesn't enforce that default itself — resolveCutGuide() below is a pure
// read of whatever design.cutGuide actually is, which is what makes a design
// saved before this feature existed (cutGuide absent, not false) correctly
// resolve to "no guide" too, same as an explicit opt-out.

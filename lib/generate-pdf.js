...
import { pageSizePtForShape, computeSheetPlacement, isWholeSheetShape, sheetFormatLabel } from './paper-config.js';
 ...
 * for icing-sheet shapes, Letter for wafer paper (lib/paper-config.js);
 * image is placed at its physical print position/size via
 * computeSheetPlacement(), the same function the print-preview modal uses.
 * Layout otherwise identical to app/api/generate-pdf/route.js.
 *
 * @param {{
...
export async function generatePrintPdf({ imageUrl, shape, sizeInches, customW, customH }) {
  ...
  // 2. Page size by material (A4 for icing sheet, Letter for wafer paper —
  ...
  // sheet format (A4/LETTER) instead, joined with " · " to read as one
  // label rather than running into the shape name.

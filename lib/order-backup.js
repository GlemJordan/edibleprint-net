import { searchOrders, fetchRawText, orderFolderPath, uploadRaw, ORDER_PUBLIC_ID_RE } from './cloudinary-ops.js';
...
const CSV_COLUMNS = [
  ...
  // Readable companion to committedDate below — same "Pickup date" / "Ship
  // by" label the admin UI and daily digest email use (both derived from
  // shipping.method the same way), so a reader doesn't have to infer which
  // one committedDate means from the raw shippingMethod value above.
  ['committedDateType', (r) => (r.shipping?.method === 'pickup' ? 'Pickup date' : 'Ship by')],
  // Meaning depends on committedDateType above — pickup date for a pickup
  // order, ship-by date otherwise (see types/order.js). Absent for any
  // order with no date entered yet.
  ['committedDate', (r) => r.committedDate || ''],
  ['designCount', (r) => r.designs?.length || 0],
  // Manual orders (source: 'manual') price the ORDER as a whole
  // (payment.amountCents, the 'amountCAD' column below) and leave each
  // design's own unitPrice at 0 — printing "($0)" here would read as "gave
  // this away for free", so the per-design price is only shown when it's a
  // real, priced line (every Stripe order design always has one).
  ['designsSummary', (r) => (r.designs || [])
    .map((d) => `${d.shapeLabel || d.shape} ${d.size} x${d.quantity}${d.unitPrice > 0 ? ` ($${d.unitPrice})` : ''} [${d.imageUrl || 'no image'}]`)
    .join(' | ')],

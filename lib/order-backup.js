import { searchOrders, fetchRawText, orderFolderPath, listOrderFiles } from './cloudinary-ops.js';
...
export async function selectPrintFilesForBackup(records) {
  ...
  for (const record of unprinted) {
    let urls = record.assets?.printReadyUrls || [];
    if (urls.length === 0) {
      // assets.printReadyUrls can be missing/stale even when the actual
      // print-ready PDF exists in Cloudinary — verified directly against
      // production data, several existing orders have the file in storage
      // with no matching order.json entry. Check storage itself before
      // concluding there's really nothing to attach.
      try {
        const found = await listOrderFiles(record.orderId, 'print-design');
        urls = found.map((f) => ({ url: f.url, label: f.publicId.split('/').pop().replace(/\.pdf$/, '') }));
      } catch (err) {
        console.error(`[order-backup] fallback file listing failed for ${record.orderId}:`, err.message);
      }
    }
    if (urls.length === 0) {

import { generateProductionSlip, generatePrintPdf, extractPdfPage, parseDesignSizeForPdf } from './generate-pdf.js';
  ...
  for (let i = 0; i < designs.length; i++) {
    ...
    try {
      const printBytes = await withRetry(
        () => generatePrintPdf({
          imageUrl: d.imageUrl, shape: d.shape, material: resolveMaterial(d), sizeInches, customW, customH,
          cutGuide: resolveCutGuide(d), customShapeKind: d.customShapeKind,
        }),

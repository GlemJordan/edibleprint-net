...
import { resolveCut } from './cutting-config.js';
import { resolveCutGuide } from './cut-guide-config.js';
 ...
 * @param {string} orderId  e.g. 'EP-4T7K' — see generateUniqueOrderId()
...
export function buildOrderRecord(session, designs, orderId, isTest) {
  ...
  const record = {
    ...
    designs: designs.map(d => ({
      ...
      material:   resolveMaterial(d),
      cutToShape: resolveCut(d),
      cutGuide:   resolveCutGuide(d),
      ...(d.shape === 'custom' && d.customShapeKind ? { customShapeKind: d.customShapeKind } : {}),
      size:       d.size || '',
      ...
      // Ready-made catalog design flow only (app/designs/[id]/page.js) —
      // additive, absent for every design from the other two flows.
      ...(d.catalogDesignId ? {
        catalogDesignId: d.catalogDesignId,
        customText:      d.customText || undefined,
      } : {}),
    })),
    shipping: {
...
// Order code alphabet — excludes I/L/O/U and 0/1, the characters most
// commonly misread or mis-dictated against each other (I/L/1, O/0/U) — so a
// customer reading a code off a package, or a support call spelling one out
// loud, doesn't hit an ambiguous character. 30 characters × 4 positions =
// 810,000 possible codes; see generateUniqueOrderId() below for why that's
// "improbable, not impossible" and handled as such rather than assumed safe.
const ORDER_ID_ALPHABET = 'ABCDEFGHJKMNPQRSTVWXYZ23456789';
const ORDER_ID_LENGTH = 4;

// One alphabet character from a uniform distribution — rejection-sampled
// rather than `randomBytes(1)[0] % 30`, which would let byte values 0-15
// land ~12.5% more often than 16-29 (256 isn't evenly divisible by 30). Not
// a security boundary (this is a human-facing lookup code, not a secret),
// but avoiding a skewed alphabet costs nothing here.
function randomOrderIdChar() {
  const REJECT_AT = 240; // largest multiple of ORDER_ID_ALPHABET.length (30) <= 256
  let byte;
  do {
    byte = randomBytes(1)[0];
  } while (byte >= REJECT_AT);
  return ORDER_ID_ALPHABET[byte % ORDER_ID_ALPHABET.length];
}

// True if an order.json already exists at this id — the check
// generateUniqueOrderId() runs before ever handing out a candidate code,
// since lib/cloudinary-ops.js's uploadRaw() always uploads with
// overwrite:1: a collision wouldn't fail loudly, it would silently replace
// the existing order. Only a 404 reads as "available" — any other failure
// (network, auth, Cloudinary outage) propagates instead of being treated as
// a green light, since guessing wrong here risks exactly the data loss this
// check exists to prevent.
async function orderIdExists(orderId) {
  try {
    await fetchRawText(`${orderFolderPath(orderId)}/order`);
    return true;
  } catch (err) {
...
// THE one place that mints a new order id — used for both Stripe orders
// (app/api/webhook/route.js) and manual ones (app/api/admin/orders/manual/
// route.js), which is new: they used to draw from disjoint id schemes (an
// 'EP-M' prefix categorically kept manual ids away from Stripe's own
// derived-from-session-id ones) specifically so collision could never
// happen. A real existence check replaces that trick now that codes are
// short enough for collision to be a genuine (if unlikely) possibility —
// both order sources share one namespace and the same guarantee.
//
// @returns {Promise<string>} a confirmed-unused 'EP-XXXX' code
export async function generateUniqueOrderId() {
  const MAX_ATTEMPTS = 20;
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    let code = '';
    for (let i = 0; i < ORDER_ID_LENGTH; i++) code += randomOrderIdChar();
    const candidate = 'EP-' + code;
    if (!(await orderIdExists(candidate))) return candidate;
    console.warn(`[order-record] order id collision on ${candidate} (attempt ${attempt}/${MAX_ATTEMPTS}), retrying`);
  }
  throw new Error(`Could not generate a unique order id after ${MAX_ATTEMPTS} attempts`);
}
 ...
 *   shape: string, material?: 'icing'|'wafer', size: string, quantity: number,
 *   amountCents: number,
...
export function buildManualOrderRecord(input, orderId) {
  ...
  // input.shape is a real catalog shape key (circular/heart/.../bwsheet/
  // custom) for every preset the admin form offers, or free text for its
  // 'Other' escape hatch — SHAPE_LABELS[input.shape] resolves the former,
  // the fallback to the raw value preserves the latter, same as every
  // other order source's shapeLabel computation.
  const design = {
    shape:      input.shape,
    shapeLabel: SHAPE_LABELS[input.shape] || input.shape,
    // resolveMaterial() (not a plain === 'wafer' check) so the free-text
    // 'Other' escape hatch — where shape is hand-typed and material has no
    // selector to fill it — still gets caught if the admin's free text
    // happens to say "wafer" anywhere. See lib/material-config.js.
    material:   resolveMaterial(input),
    size:       input.size || '',
...
export function deriveSearchContext(record) {
  return {
    ...
    // Derived to a plain boolean rather than passing shipping.method through
    // as-is — Stripe orders write 'canada_post_shipping', manual orders
    // write 'local_delivery'/etc, none of which match the ProductionStatus-
    // style enum in types/order.js's typedef; the admin list only ever needs
    // "is this a pickup order" (to label the delivery-date column pickup vs
    // ship-by), so collapsing the inconsistency here means it only has to be
    // understood in one place.
    isPickup:         record.shipping?.method === 'pickup' ? 'true' : 'false',
    // Empty string (not omitted) when unset, matching how
    // updateCommittedDate() clears it — Cloudinary's context 'add' command
    // merges keys, so a key that's simply never written keeps its last
    // value forever otherwise.
    committedDate:    record.committedDate || '',
    // Needed alongside committedDate for the list page to compute urgency
    // (see lib/delivery-urgency.js's isOrderResolved) without an N+1 fetch
    // of every order body — payment.status is already effectively public
    // (shown on the detail page) so there's no new exposure here.
    paymentStatus:    record.payment?.status || 'paid',
  };
}
...
export async function updateOrderStatus(orderId, newStatus, adminNote) {
  ...
  // Resync the FULL searchable context from the just-patched record, not
  // just {status: newStatus} — the admin order list reads context only,
  // never the full body. A narrow single-field patch was the actual bug
  // behind "every pre-existing order shows the wrong pickup/ship-by label":
  // deriveSearchContext() gained isPickup/committedDate/paymentStatus after
  // this function already existed, and a patch that only ever wrote
  // {status} could never backfill them onto an order it touched. Resyncing
  // everything here means any order this function runs on self-heals for
  // every context field, present or future, not just the one this call
  // cares about.
  await updateResourceContext(jsonPublicId, deriveSearchContext(record));
...
/**
 * Set (or clear) an order's committedDate — pickup date for a pickup order,
 * ship-by date otherwise (see types/order.js). Same read-modify-write +
 * poll-until-visible pattern as updateOrderStatus() above, for the same
 * Cloudinary raw-storage staleness reason.
 *
 * @param {string} orderId
 * @param {string|null} committedDate  YYYY-MM-DD, or null to clear it
 * @returns {Promise<import('../types/order.js').OrderRecord>}
 */
export async function updateCommittedDate(orderId, committedDate) {
  const folder = orderFolderPath(orderId);
  const jsonPublicId = `${folder}/order`;
  ...
  if (committedDate) {
    record.committedDate = committedDate;
  } else {
    delete record.committedDate;
  }

  await uploadRaw(JSON.stringify(record, null, 2), jsonPublicId);
  // Full context resync, same reasoning as updateOrderStatus() above — not
  // just {committedDate}, so this call also backfills isPickup/paymentStatus
  // for any order that predates those fields, instead of leaving them
  // permanently absent from context until something else happens to patch
  // them in. deriveSearchContext() already reads committedDate as '' when
  // absent, which is what a clear (committedDate === null here) needs.
  await updateResourceContext(jsonPublicId, deriveSearchContext(record));

  for (let attempt = 0; attempt < 5; attempt++) {
    const check = JSON.parse(await fetchRawText(jsonPublicId));
    if ((check.committedDate || null) === (committedDate || null)) return check;
    await new Promise((r) => setTimeout(r, 400 * (attempt + 1)));
  }
  console.warn(`[order-record] committedDate update for ${orderId} not confirmed visible after retries — returning optimistic record`);
  return record;
}

...
export async function updateCommittedDate(orderId, committedDate) {
  ...
  await uploadRaw(JSON.stringify(record, null, 2), jsonPublicId);
  // Keep the searchable context in sync the same way status updates do —
  // the admin order list reads context only, never the full body. Empty
  // string (not omitting the key) clears a previously-set value: Cloudinary's
  // context 'add' command merges keys in, it doesn't clear ones left out.
  await updateResourceContext(jsonPublicId, { committedDate: committedDate || '' });

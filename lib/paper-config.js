...
//
// fullsheet's bottom is 10mm, not 1mm like waferletter — this business's
// Canon printer leaves dark banding in the last ~2-2.5cm of the sheet
// (roller slip at the end of the feed path) and ink can reach the sheet's
// non-edible plastic backing there. Deliberately separate objects now
// (previously one shared WHOLE_SHEET_MARGIN_MM, by design, so the two could
// never drift apart) — this printer artifact is fullsheet-specific, so
// waferletter keeps the original 1mm bottom rather than losing print area
// it doesn't need to.
//
...
// design. At top=15mm/bottom=1mm (waferletter) the resulting side margin is
// ~5.66mm; at top=15mm/bottom=10mm (fullsheet) it's ~8.84mm — raising bottom
// tightens the height/width scale mismatch further, so the sides grow along
// with it (design area drops from ~558cm² to ~523cm², about 6.3%).
const FULLSHEET_MARGIN_MM = { top: 15, right: 1, bottom: 10, left: 1 };

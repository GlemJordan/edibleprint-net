...
// Note: right/left barely move the rendered side margin in practice — with
// top fixed at 20mm, the uniform "scale to fit, never crop/distort" scale
// factor in computeSheetPlacement() is height-bound (top+bottom eat far
// more of the sheet's proportional room than left+right ever could here),
// so the actual left/right whitespace is a side effect of that scale, not
// a direct reading of these two numbers. bottom, by contrast, is exact:
// whatever's set here is exactly what's left under the design. Set to the
// practical minimum (1mm, not 0) to keep a hair of clearance at the
// trailing edge while pushing the resulting side margins down to their
// floor (~7.4mm) given the fixed top.
const SHEET_MARGIN_MM = {
  fullsheet: { top: 20, right: 1, bottom: 1, left: 1 },
};

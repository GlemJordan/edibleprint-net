 ...
 * @returns {string} 'EP-XXXXXXXX' -- deliberately its OWN scheme, distinct
 * from real orders' random 'EP-XXXX' code (see generateUniqueOrderId(),
 * lib/order-record.js): a standalone digital-download purchase never gets
 * an order record of its own, so this is just a stand-in label for its PDF
 * filename, not a real lookup key -- it doesn't need collision-checking
 * (nothing is ever stored under it) and deriving it straight from the
 * session id (instead of a random draw) keeps it that way.
 */
